...
## 增量交付记录（2026-04-08：资产删除能力补齐）

### 背景

- 在真实使用中发现“历史开发资产无法删除”，会影响后续实验数据集清理与重建。

### 本轮新增能力

- 后端新增 `DELETE /api/assets/{asset_id}`：
  - 删除资产本体
  - 级联删除关联解析、知识库、问答、导图、笔记、演示记录
  - 尝试清理 OSS 上的显式 key 与资产前缀对象
  - 返回删除结果与告警（如 OSS 未配置或部分文件清理失败）
- 前端图书馆页新增“删除资产”按钮，支持二次确认与删除态提示。

### 删除策略说明

- **数据库策略**：以 `Asset` 为根进行级联删除（依赖现有关系 `cascade="all, delete-orphan"`）。
- **存储策略**：双层清理，先删显式记录 key，再按资产前缀兜底扫描删除，尽量避免遗漏。
- **容错策略**：若 OSS 不可用，优先保证数据库可删除，并通过 `warning` 回传人工排障信息。

### 本轮验证

- 单测通过：`tests/test_asset_delete_service.py`（2 tests）
- 前端构建通过：`npm run build`
- 手工验证通过：已成功删除历史 `transformer` 资产，API 返回 `deleted=true`。

### 后续建议

- 若后续需要“误删恢复”，可在此基础上引入软删除与延迟清理队列。
