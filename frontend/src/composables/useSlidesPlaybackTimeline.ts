...
function resolvePagePlan(
  page: SlideDslPage | undefined,
  pagePlans: SlidePlaybackPagePlan[],
): SlidePlaybackPagePlan | null {
  if (!page) {
    return null;
  }
...
export function useSlidesPlaybackTimeline(options: {
  pages: Ref<SlideDslPage[]>;
  currentSlideIndex: Ref<number>;
  playbackPlan: Ref<SlidePlaybackPlan | null>;
}) {
  ...
  const previewGlobalMs = ref<number | null>(null);
  ...
  const committedGlobalMs = computed(() => {
    ...
    return clamp(
      currentPagePlan.value.start_ms + currentPageElapsedMs.value,
      0,
  ...
  function setAutoPageEnabled(value: boolean) {
    autoPageEnabled.value = value;
  }

  function syncToSlideStart(index: number) {
  ...
  function setCurrentPageElapsedMs(value: number) {
  ...
  function beginPreview(globalMs: number) {
    previewGlobalMs.value = clamp(Math.round(globalMs), 0, totalDurationMs.value);
  }

  function endPreviewAndGetSeekTarget(): number {
    const target = previewGlobalMs.value ?? committedGlobalMs.value;
    previewGlobalMs.value = null;
    return target;
  }

  function seekGlobalMs(globalMs: number): { pageIndex: number; pageElapsedMs: number } {
    ...
    const matched = pagePlans.value.find(
      (item) => clamped >= item.start_ms && clamped < item.end_ms,
    ) ?? pagePlans.value[pagePlans.value.length - 1];
